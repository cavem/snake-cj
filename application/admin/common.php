...
function getuname($id){
    $uname = Cache::get("uname_".$id);

    if(!$uname){
        $uname = db('users')->where("id",$id)->value('nickname');
        Cache::set("uname_".$id,$uname);
    }
...
function getgname($id){
    $gname = Cache::get("gname_".$id);

    if(!$gname){
        $gname = db('game')->where("id",$id)->value('title');
        Cache::set("gname_".$id,$gname);
    }
...
function getRewardname($id){
    ...
    if($id){
        foreach($id as $val){
            $content = db('game_reward')->where('id',$val)->value('content');
            $rewardname.=$content.'</br>';
        }
    }
